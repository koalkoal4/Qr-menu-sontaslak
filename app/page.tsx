...
// Business ve Category tiplerini içe aktarıyoruz, Profile artık kullanılmıyor.
import { Business, Category } from '@/lib/types';
import { Skeleton } from '@/components/ui/skeleton';

// MenuHeader bileşeni artık Business tipini alacak şekilde güncellendi.
function MenuHeader({ business }: { business: Business | null }) {
    if (!business) {
        return <Skeleton className="w-full h-48 mb-6" />;
    }
    
    const coverImageUrl = business.cover_image_url
        ? `${process.env.NEXT_PUBLIC_SUPABASE_URL}/storage/v1/object/public/cover-images/${business.cover_image_url}`
        : null;
    ...
    return (
        <div className="relative mb-6">
            <div className="w-full h-80 overflow-hidden">
                <Image src={coverImageUrl} alt={`${business.name} Kapak Fotoğrafı`} fill className="object-cover"/>
            </div>
            {business.instagram_url && (
                <a 
                  ...
                  className="absolute top-3 right-3 w-12 h-12 flex items-center justify-center bg-black/50 rounded-full hover:bg-black/70 transition-colors"
                >
...
export default function HomePage() {
  ...
  // State artık tek bir business nesnesi tutacak
  ...
  useEffect(() => {
    const fetchBusinessData = async () => {
      ...
      if (!businessId) {
        setError("İşletme kimliği (business ID) bulunamadı. Lütfen ortam değişkenlerini kontrol edin.");
        setIsLoading(false);
        return;
      }
      ...
      // Tek bir sorgu ile işletme ve ilişkili kategorileri çekiyoruz
      const { data: businessData, error: fetchError } = await supabase
        ...
        `)
        .eq('id', businessId)
        .eq('categories.is_available', true) // Sadece aktif kategorileri çek
  ...
  return (
    <div>
      {/* MenuHeader'a artık business nesnesini doğrudan gönderiyoruz */}
      <MenuHeader business={business} />
      
      <div className="p-4">
        <header className="text-center mb-8">
            {/* İşletme adını dinamik olarak gösteriyoruz */}
        ...
        <div className="grid grid-cols-2 gap-4">
            {isLoading ? (
              ...
              // Kategorileri business nesnesinden alıp map ediyoruz
              business?.categories?.map(category => (
                  <CategoryCard key={category.id} category={category} />
              ))
            )}
        </div>
        {/* İşletme veya kategori bulunamazsa gösterilecek mesaj */}
